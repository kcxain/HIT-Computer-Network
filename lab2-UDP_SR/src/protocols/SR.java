...
public class SR {
    ...
    public SR(String host, int targetPort, int ownPort) throws UnknownHostException {
    ...
    public void send(byte[] content) throws IOException {
        ...
        do {
            while (timers.size() < WindowSize && sendIndex < content.length && sendSeq < 256) { // until the window is run up
                ...
                temp = new byte[1];
                temp[0] = valueOf((int) sendSeq).byteValue();
                oneSend.write(temp, 0, 1);
            ...
            for (int i = 0; i < timers.size(); i++) { // update timer
                int sendMaxTime = 2;
                if (timers.get(i) > sendMaxTime) { // resend the datagram which hasn't receive ACK and over time
    ...
    public ByteArrayOutputStream receive() throws IOException {
        ...
        while (true) {
            ...
            try {
                ...
                // this value is used to simulate the loss of the datagram as a remainder
                int virtualLossRemainder = 17;
